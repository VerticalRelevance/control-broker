...
class RequestParser:
    ...
    def get_approved_context(self):
        def integrate_with_my_entitlement_system(
            consumer_metadata, consumer_request_context
        ):
        ...
        if integrate_with_my_entitlement_system(
            self.consumer_metadata, self.consumer_request_context
        ):
...
def sign_request(
    *,
    ...
    auth = BotoAWSRequestsAuth(
        aws_host=host, aws_region=region, aws_service="execute-api"
    )

    print(f"begin request\nfull_invoke_url\n{full_invoke_url}\njson input\n{input}")

    r = requests.post(full_invoke_url, auth=auth, json=input)

    print(f"signed request headers:\n{dict(r.request.headers)}")

    content = json.loads(r.content)

    r = {"StatusCode": r.status_code, "Content": content}
...
def put_object(*, bucket, key, object_: dict):
    ...
    try:
        r = s3.put_object(Bucket=bucket, Key=key, Body=json.dumps(object_))
...
def lambda_handler(event, context):
        ...
        "ControlBrokerEvaluation": {
            "Raw": {
                "PresignedUrl": generate_presigned_url(
                    bucket=os.environ["RawPaCResultsBucket"], key=evaluation_key
                ),
    ...
    control_broker_request_status = {
        "Request": {
            ...
            "Requestor": {"IsAuthorized": r.requestor_is_authorized},
            "InputType": {"Validated": bool(r.validated_input_type)},
            "Context": {"IsApproved": bool(r.approved_context)},
        },
